...
const { GutenBook } = require("../gutenmodel")
...
router.get("/get", (req, res, next) => {
    ...
    if (source == "gutenberg") {
        (async () => {
            const result = await GutenBook.findOne({
                where: {
                    bookId: id
                },
                include: ["Formats"]
            ...
            for (const format of result.Formats) {
                formats[format.mimeType] = format.url;
            }
            let url = formats["text/plain"];
            if (!url?.endsWith("txt")) {
                url = formats["text/plain; charset=utf-8"];
            }
            if (!url?.endsWith("txt")) {
                url = formats["text/plain; charset=us-ascii"];
            }
            if (!url?.endsWith("txt")) {
                url = formats["text/plain; charset=utf-8"]?.replace(/zip$/, "txt");
            }
            if (!url) {
